...
function getEffectiveBaseDate(profile, todayDate) {
  if (profile.todayPeriodStartEnabled) return todayDate;
...
function buildCycleStarts(profile, rangeStart, rangeEnd, todayDate) {
  ...
  for (index = backwardCount; index >= 1; index -= 1) {
    starts.push({ cycleIndex: -index, startDate: addDays(baseDate, -index * cycleLength), isPredictedStart: false, source: "history" });
  }
  ...
  for (index = 1; index <= forwardCount; index += 1) {
    ...
    if (adjustedDate && diffDays(previousStart, adjustedDate) > 0) finalStart = adjustedDate;
...
function classifyDate(date, cycleStarts, periodLength) {
  ...
  for (index = 0; index < cycleStarts.length; index += 1) {
    ...
    if (daysBeforeStart === 14) {
      return { status: "ovulation", statusLabel: "参考日", cycleIndex: cycle.cycleIndex, cycleStartDateKey: formatDateKey(cycle.startDate) };
    }
    if (daysBeforeStart >= 13 && daysBeforeStart <= 19) {
      return { status: "fertile", statusLabel: "参考窗口", cycleIndex: cycle.cycleIndex, cycleStartDateKey: formatDateKey(cycle.startDate) };
    }
  }

  return { status: "safe", statusLabel: "其他日期", cycleIndex: null, cycleStartDateKey: "" };
}
...
function buildWeeks(profile, year, month, selectedDateKey) {
  ...
  var weeks = [];
  for (index = 0; index < cells.length; index += 7) weeks.push(cells.slice(index, index + 7));
  return { weeks: weeks, cycleStarts: cycleStarts };
}
