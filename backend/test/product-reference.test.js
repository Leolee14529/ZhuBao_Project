...
test("detail page swipes multi-image works without a journal entry card", () => {
  ...
  assert.doesNotMatch(detailLogic, /openDailyRecord|require\("\.\.\/\.\.\/\.\.\/\.\.\/utils\/auth"\)/);
  ...
  assert.doesNotMatch(detailView, /记录今日感受|recordFeeling|openDailyRecord|record-button/);
  assert.doesNotMatch(copy, /recordFeeling:\s*"记录今日感受"/);
  assert.doesNotMatch(copy, /recordFeeling:\s*"Record today's feeling"/);
});
